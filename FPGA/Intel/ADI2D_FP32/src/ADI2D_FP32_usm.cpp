...
using rd_pipe = ext::intel::pipe<class pVec16_r, dPath16, 8>;
using wr_pipe = ext::intel::pipe<class pVec16_w, dPath16, 8>;
    ...
    h.single_task<class PipeConvert_512_256>([=]() [[intel::kernel_args_restrict]] {
      ...
      for (int i = 0; i < total_itr; i++) {
        ...
        for (int v = 0; v < VFACTOR; v++) {
          ...
          case 2: {
            data.data[v] = data16.data[v + VFACTOR * 2];
            break;
          }
          case 3: {
            data.data[v] = data16.data[v + VFACTOR * 3];
            break;
          }
          }
        }
    ...
    h.single_task<class pipeConvert_256_512>([=]() [[intel::kernel_args_restrict]] {
      ...
      for (int i = 0; i < total_itr; i++) {
        ...
        for (int v = 0; v < VFACTOR; v++) {
          ...
          case 2: {
            data16.data[v + VFACTOR * 2] = data.data[v];
            break;
          }
          case 3: {
            data16.data[v + VFACTOR * 3] = data.data[v];
            break;
          }
          }
        }
